...
/*
	...
	cout<<"sqlite3_exec执行时间为： "<<double((Endtime.QuadPart-Starttime.QuadPart)*1000/Freq.QuadPart)<<endl;
	free(pPath);
	sqlite3_close(db);
	return nResult;
}
...
int WriteDB_Multi()
{
	...
	try {
		...
		PR_snprintf(szSqlBuffRept, sizeof(szSqlBuffRept),          
			"insert into alarminfo(type,errorcode,IsSolved,starttime,ip,moudle) values( %d,%d,%d,datetime('now','localtime'),'%s', '%s');",1,
			1,1,"2.2.2.9","插入多条数据");
		szAlarmSql = ConvertToUTF(szSqlBuffRept);
